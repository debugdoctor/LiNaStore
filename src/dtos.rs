...
mod tests {
    ...
    fn test_payload_new() {
        ...
        assert_eq!(payload.identifier.len(), 255);
    ...
    fn test_lina_protocol_new() {
        ...
        assert_eq!(protocol.payload.identifier.len(), 255);
        assert_eq!(protocol.payload.length, 0);
    ...
    fn test_serialize_protocol_message() {
        ...
        // Status (1 byte) + identifier (255 bytes) + length (4 bytes) + checksum (4 bytes) + data (5 bytes)
        assert_eq!(serialized.len(), 1 + 255 + 4 + 4 + 5);
    ...
    fn test_payload_with_large_data() {
        ...
        protocol.payload.length = large_data.len() as u32;
        protocol.payload.checksum = protocol.calculate_checksum();
        
        assert!(protocol.verify());
        assert_eq!(protocol.payload.length, 100000);
    }
    ...
    fn test_serialize_deserialize_roundtrip() {
        ...
        let data_start = 1 + 255 + 4 + 4; // status + identifier + length + checksum
